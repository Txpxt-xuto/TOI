...
int main()
{
    ...
    dp[0][0]=0;
    for(int i=1;i<=n;i++) for(int x=500;x>=point[i].first;x--) for(int y=500;y>=point[i].second;y--) dp[x][y]=min(dp[x][y],dp[x-point[i].first][y-point[i].second]+cost[i]);
    ll ans=1e18+7;
...
int main()
{
    ...
    int n;
    cin >> m >> n >> t;
    for(int i=1;i<=t;i++) for(int j=1;j<=m;j++) cin >> road[i][j];
    canFinish(0, n);
...
void ADSL(vector<int> &room, int m)
{
    ...
    while(low<high)
    {
        if(canSplit(room, low, high, m)) high = (low+high)/2;
...
void Menu()
{
    printf("ดำเนินการจองรถ");
    printf("ยกเลิกการจองรถ");
    printf("ดูรถที่ว่างกับวัน เวลาของคุณ");
    printf("อ่านเงื่อนไข กติกาการเช่า");
